...
@section('title', 'Edit delivery rule — Admin')
@section('heading', 'Edit delivery rule')

@section('content')
    <form action="{{ route('dashboard.delivery-rules.update', $rule) }}" method="post" class="mt-6 max-w-xl space-y-5">
        @csrf
        @method('PUT')
        <div>
            <label for="zone" class="block text-sm font-medium text-slate-700">Zone <span class="text-red-600">*</span></label>
            <input type="text" name="zone" id="zone" value="{{ old('zone', $rule->zone) }}" required class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-slate-900 shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500">
        ...
        <div>
            ...
            <select name="method" id="method" required class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-slate-900 shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500">
                @foreach (['rider', 'driver', 'third_party', 'pickup', 'manual'] as $m)
                    <option value="{{ $m }}" @selected(old('method', $rule->method) === $m)>{{ $m }}</option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <select name="option" id="option" required class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-slate-900 shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500">
                @foreach (['standard', 'express', 'pickup'] as $o)
                    <option value="{{ $o }}" @selected(old('option', $rule->option) === $o)>{{ $o }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label for="price" class="block text-sm font-medium text-slate-700">Price (GHS) <span class="text-red-600">*</span></label>
            <input type="number" name="price" id="price" value="{{ old('price', $rule->price) }}" step="0.01" min="0" required class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-slate-900 shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500">
        </div>
        <div>
            <label for="estimated_time" class="block text-sm font-medium text-slate-700">Estimated time</label>
            <input type="text" name="estimated_time" id="estimated_time" value="{{ old('estimated_time', $rule->estimated_time) }}" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-slate-900 shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500">
        </div>
        <div class="flex items-center gap-2">
            <input type="hidden" name="active" value="0">
            <input type="checkbox" name="active" id="active" value="1" class="rounded border-slate-300 text-slate-900 focus:ring-slate-500" @checked(old('active', $rule->active))>
